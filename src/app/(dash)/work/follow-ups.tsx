...
/**
 * Lead follow-ups, bucketed by when they are due.
 *
 * Lifted out of the old Agenda page so the same view can live as a tab
 * alongside the rest of somebody's day, instead of being a separate
 * destination that showed a slice of what My work already showed.
 */
export default async function FollowUps({ scope }: { scope: number | null }) {
  let rows: LeadRow[] = [];
  let failed = false;
  ...
  return (
    <>
      {!failed && (
        <p className="board-hint" style={{ marginBottom: 8 }}>
          <b>{booked}</b> follow-up{booked === 1 ? "" : "s"} booked
          {overdueCount > 0 && (
            <>
              {" · "}
              <b style={{ color: "var(--err)" }}>{overdueCount}</b> overdue
            </>
          )}
        </p>
      )}
